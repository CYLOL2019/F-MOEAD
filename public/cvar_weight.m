function pf = cvar_weight(combination, weight)
global mocpo_params
...
% % Objective
portprob.Objective = [CVaR,PR] * weight;

%% Solve weight decomposition
[sol,fval] = solve(portprob,'Options',options);
pf = zeros(2,1);
pf(1) = sol.VaR + pr'*sol.VaRdev/(1-alpha);
pf(2) = - ep * sol.portfolio * roundlot;
